...
// 选择保存在当前浏览器，不依赖 DSH Host 的设置白名单。
...
var CURRENCY_STORAGE_KEY = 'dsh-cost-log.currency';
...
function readStoredCurrency() {
  try {
    ...
    return CURRENCIES.indexOf(saved) === -1 ? DEFAULT_CURRENCY : saved;
  } catch (_) {
...
function apply(ctx) {
  ...
  var currencyStore = createSnapshotStore(readStoredCurrency());
