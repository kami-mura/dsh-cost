 ...
 */
export function pricePointAt(model, time) {
  const family = familyOf(model)
  if (family === null) return null
  const at = Number.isFinite(time) ? time : Date.now()
  if (at < EFFECTIVE_AT_MS) {
    return { mode: 'legacy', ...LEGACY_RATES[family] }
  }
  const peak = isPeakAt(at)
  const tier = peak ? NEW_RATES[family].peak : NEW_RATES[family].offpeak
  return { mode: peak ? 'peak' : 'offpeak', ...tier }
}

/** 把浮点金额固定到 1e-10 元，避免二进制浮点尾巴。 */
...
function shiftModel(byModel, model, buckets, sign, rate) {
...
function foldUsage(state, sample) {
  ...
  if (prev !== null) {
    ...
    byModel = shiftModel(byModel, prev.model, prev.buckets, -1, prevRate)
  }
  ...
  byModel = shiftModel(byModel, sample.model, buckets, 1, rate)
